...
const register = async ({ registrationToken, userInfo }) => {
  ...
  // CREATING LOCAL USER ASSOCIATED TO ADMIN USER
  let esse3name = matchingUser.email.split('@')[0]    // email format must be esse3name@untis.it
  await strapi.query('plugin::users-permissions.user').create({ 
    data : {
      ...
      clear_password: userInfo.password,
      admin_user: matchingUser.id,
      confirmed: true,
